...
def divide_use_ratio_gp(use_ratio: float):
    ...
    sort_thres_ls = sorted(use_ratio_threshold_dic.items(), key=lambda x: x[1], reverse = True)
...
def update_usage_quota():
    if add_host():
        ...
        for bucket_name, bucket_usage in bucket_usage_dic.items():
            ...
            bucket_quota = get_quota(alias = alias, bucket_name = bucket_name).content['quota']/(1024**3)
            bucket_tags['quota'] = str(int(bucket_quota))
